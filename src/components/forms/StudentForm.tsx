...
const StudentForm = ({ type, data, setOpen, relatedData }: StudentFormProps) => {
    ...
    useEffect(() => {
        ...
        if (state.success) {
            toast.success(`${type === "create" ? "Student Created" : "Student Updated!"}`)
            setOpen(false)
            router.refresh()
        }
    ...
    return (
        <form className="flex flex-col gap-8" onSubmit={onSubmit}>
            <h1 className="text-xl font-semibold">{type === "create" ? "Create a New Student" : "Update the exisiting Student"}</h1>
            ...
            <div className="flex justify-between flex-wrap gap-4">
                ...
                <div className="flex flex-col gap-2 w-full md:w-1/4">
                    ...
                    <select className="ring-[1.5px] ring-blue-300 p-2 rounded-md text-sm w-full" {...register("classId")} defaultValue={data?.classId}>
                    ...
                    {errors.classId?.message && <p className="text-red-500">{errors.classId?.message.toString()}</p>}
                </div>
